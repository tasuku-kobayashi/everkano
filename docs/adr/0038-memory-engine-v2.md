...
- MVP の並行抽出（`memory_extraction.ja.txt`・`services/memory.py` の抽出）は **廃止** した（ADR-0009 の該当部分を置き換え。`app/services/memory.py` は互換のための再公開だけ）。
...
- 書き込みの前の安全装置（1 トランザクション・ペア単位のアドバイザリーロック。監査ログはコミットの後にまとめて書く）:
  ...
  4. `add` は重要度が `MEMORY_IMPORTANCE_THRESHOLD`（既定 0.6）以上のときだけ。`update` / `supersede` は重要度に関係なく適用する。約束の記憶は重要度 0.8 以上にする
...
- 設定・関係・評価を書き換えようとする発言は、分析の前にふるい落とし、分析の出力からも取り除く（記憶経由の注入の防止。`engine/memory/guard.py`。
  [ADR-0039](0039-user-edited-memory-protection.md)）。
...
- 候補はペアの中の厳密検索（`MATERIALIZED` CTE。[ADR-0005](0005-vector-index-and-exact-memory-search.md)）で上位 50 件を取り出し、総合点で並べ替える
  （`engine/memory/ranking.py`）:
  **総合点 = 0.55 × 類似度 + 0.2 × 重要度 + 0.15 × 新しさ + 0.1 × 語の一致 + 種類の加点**（関係性 +0.08・約束 +0.05・要約 −0.05）。
  語の一致は、質問の内容語（漢字・カタカナ・英数字の 2 文字以上）のうち記憶に含まれる割合（固有名詞の完全一致を拾う。埋め込みの弱点を補うハイブリッド検索）。
  値は `engine/memory/config.py`（`RankingWeights` / `DEFAULT_KIND_BOOST` / `DEFAULT_HALF_LIFE_DAYS` / `MemoryConfig`）。
- 新しさ = 0.5 ^ (経過日数 / 種類ごとの半減期)。半減期は事実 180 日・好み 120 日・関係性 365 日・出来事 30 日・約束 30 日・気持ち 7 日・要約 60 日。
  経過日数は「最後にプロンプトへ入れた日時」と作成日時の新しい方から数える（参照された記憶は忘れにくい）。
  **減衰はランキングだけに効き、時間の経過で記憶を消すことはない**。記憶が消えるのは、ユーザーの削除と件数の上限による入れ替えだけ。
- 常に入れる記憶: 呼び方・距離感（`relationship`）2 件、重要度 0.8 以上の事実 3 件（MemGPT の core memory に相当）、最新の要約 2 件。残りは関係の深いもの
  （類似度 0.15 以上）から、枠が余れば総合点の順に、10 件・1,200 字まで入れる。期日が今日 ± 2 日（日本時間）の約束は最大 5 件を別枠で入れる。
- キャラ側の記憶（M8・C9）: そのペアのキャラの発言と、全ユーザー共通の予定由来の出来事（直近 14 日）から、質問に近いもの（新しさの半減期 7 日）を 6 件・500 字まで入れる。
