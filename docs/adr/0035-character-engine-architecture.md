...
- 関連: エンジン仕様書 §2（E1〜E9）・§3・§8・§10・§13 / [ADR-0008](0008-llm-embedding-providers-and-mock.md)・[ADR-0013](0013-audit-log.md)・[ADR-0020](0020-content-seed-generation.md)（本 ADR で追補）・
  [ADR-0028](0028-llm-input-budgets-and-summary-retries.md)（DM 履歴の上限の値を本 ADR により置き換え）・[ADR-0036](0036-engine-job-queue-and-scheduler.md)〜[ADR-0046](0046-engine-cost-and-latency.md) /
...
- セクションごとの上限（トークンの代わりに文字数）を `ContextBudget` で持つ: ペルソナ（静的）・世界の時間と状態・関係・記憶（件数と文字数）・
  キャラ側の記憶・約束・短期の履歴。値は評価ハーネスで測って決める調整値で、`ContextBudget` の既定値が正（使った文字数は `ContextBundle.budget_report`
  → 監査ログ `chat.response` の `context_budget`）。
- **プロンプトの順序は、変わらない部分を先に、毎回変わる部分を最後に** する（DeepSeek の自動のプレフィックスキャッシュを効かせるため。`services/prompt.py` の
  `chat_messages`）:
  1. system = 静的なペルソナと守ること（`dm_system.ja.txt` の `=== user ===` より前。キャラごとに毎回同じ）
  2. 直近の会話（user / assistant。前回のリクエストの末尾まで同じ）
  3. 最新の user メッセージ = 〔今の状況〕〜〔/今の状況〕（世界の時間・今の状況・ふたりの関係・覚えていること・自分について話したこと・約束。毎回変わる）+ 今回の発言
  利用者の発言の中に書かれた〔今の状況〕の印は無害化する（システムが付けた情報に見せかけさせない）。履歴の先頭は「目印の発言」
  （`ContextBudget.history_anchor_every`）にそろえ、古い側を落としても先頭が毎ターンずれないようにする。
