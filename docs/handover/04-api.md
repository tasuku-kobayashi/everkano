...
| `GET /memories?character_id=` | 200         | —                                    | 自分とそのキャラの記憶（重要度 desc → 新しい順、最大 `MEMORY_MAX_PER_CHARACTER` + 50 件。要約は上限を超えて保存されることがあるため）`{memories: MemoryDTO[]}`                                |
| `POST /memories`              | 201         | `RATE_LIMIT_MEMORIES_PER_MINUTE`（30、PATCH と共有） | `{character_id, content, importance?, tags?}` → `MemoryDTO`（`is_user_edited = true`、重要度の既定 0.7）。ペアの記憶が `MEMORY_MAX_PER_CHARACTER`（500）件に達していたら 422 |
| `PATCH /memories/{id}`        | 200         | 同上                                 | `{content?, importance?, tags?}`（1 つ以上必須）→ `MemoryDTO`（`is_user_edited = true`。本文が変わったら再埋め込み）  |
...
| 404  | `not_found`          | 他人の / 存在しない会話・記憶・投稿・コメント、他人のコメントへの `/comments/generate`、無効なキャラ、未定義のパス（405 も `not_found`） | エラー表示                                              |
...
- OpenAPI では全認証付きエンドポイントに 413 / 429 / 503 が載っているが、実際にレート制限しているのは `/chat`、`/comments`・`/comments/generate`、
  `POST` / `PATCH /memories` だけ。
