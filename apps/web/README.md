...
- 「〇〇があなたのことを覚えました」は Realtime（`memories` の INSERT）で出す（記憶は返答の後に非同期で作られる）。
  既定では**会話が止まって約 3 分後**に出る（API の `ENGINE_POST_TURN_DELAY_SECONDS` = 180 秒。話し続けると後ろへずれ、最大 18 分）。
  返答の直後に出ないのは仕様。購読は DM の画面（`useMemoryCreatedRealtime`）なので、それまでに画面を離れると通知は出ない
  （記憶はメモリパネルに入る）。手元ですぐ確かめるなら `apps/api/.env` で `ENGINE_POST_TURN_DELAY_SECONDS=1`
  （[ADR-0046](../../docs/adr/0046-engine-cost-and-latency.md)）。
